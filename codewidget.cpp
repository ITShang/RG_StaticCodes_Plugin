...
void CodeWidget::selectFile(bool)
{
    ...
    {
        ui->showFileList->setText(tr("共选中")
    ...
    {
        ui->statusBar->showMessage(tr("取消选择，没有选择文件。"));
        return;
    }
...
void CodeWidget::selectDir(bool)
{
    ...
    if(path == NULL)
    {
        ui->statusBar->showMessage(tr("取消选择，没有选择文件夹。"));
        return;
    }
    ui->showFileList->setText(tr("选中文件夹目录：")+path);
    filesList.clear();
...
void CodeWidget::lookFileContents(bool)
{
    ...
    {
        ui->statusBar->showMessage(tr("没有可以显示的文件"));
        return;
    }
    ...
    if(row<0)
    {
        ui->statusBar->showMessage(tr("未选择可以操作的文件目录"));
        return;
    }
...
void CodeWidget::updateTable(QString fileName,int totleLines,int codeLines,int noteLines,int spaceLines)
{
    ...
    ui->statusBar->showMessage(fileName+tr(" 统计完成"));
...
void CodeWidget::lookForFile(const QString &path)
{
    ...
    ui->showFileList->setText(tr("选中文件夹目录：")
                              + path
                              + tr("共添加文件")
                              + QString::number(filesList.count())
                              + tr("个"));
...
QString CodeWidget::makeHtmlSontents()
{
    ...
    preMess += tr("<font face=\"楷体_GB2312\" size=\"5\">统计结果");
    ...
    preMess += tr("<tr>");
    preMess += tr("<td width=\"15%\" align=\"center\"><font color=\"#800080\">文件名称</font></td>");
    preMess += tr("<td width=\"15%\" align=\"center\"><font color=\"#800080\">总行数</font></td>");
    preMess += tr("<td width=\"15%\" align=\"center\"><font color=\"#800080\">代码行数</font></td>");
    preMess += tr("<td width=\"15%\" align=\"center\"><font color=\"#800080\">注释行数</font></td>");
    preMess += tr("<td width=\"15%\" align=\"center\"><font color=\"#800080\">空白行数</font></td>");
    preMess += tr("</tr>");
